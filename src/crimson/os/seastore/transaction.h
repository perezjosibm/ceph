...
struct phase_latency_stats_t {
  ...
  void add(phase_latency_bucket_t bucket, duration_t dur) {
    ...
    assert(idx < total_ns.size());
    ++sample_counts[idx];
    total_ns[idx] += to_ns(dur);
  }

  void enter(phase_latency_bucket_t bucket) {
    auto idx = static_cast<std::size_t>(bucket);
    assert(idx < active_depth.size());
    ++active_depth[idx];
  }

  void exit(phase_latency_bucket_t bucket, duration_t dur) {
    auto idx = static_cast<std::size_t>(bucket);
    assert(idx < active_depth.size());
  ...
  public:
    phase_latency_timer_t(Transaction &t, phase_latency_bucket_t bucket)
      : stats(&t.phase_latency_stats),
        bucket(bucket),
        start(phase_latency_stats_t::clock::now()) {
      stats->enter(bucket);
    }
    ...
    ~phase_latency_timer_t() {
      if (stats != nullptr) {
        stats->exit(bucket, phase_latency_stats_t::clock::now() - start);
      }
    }
  private:
    phase_latency_stats_t *stats = nullptr;
    phase_latency_bucket_t bucket;
    phase_latency_stats_t::clock::time_point start;
  };
